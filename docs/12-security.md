# Part 12: System Security Hardening

In this guide, you will lock down your collaborative app to prevent unauthorized access. You will build a secure Socket handshake authenticator that decodes user JWTs, implement strict backend database validation checks before allowing users to enter editor rooms, and configure rate limit defenses.

---

## 1. WebSockets JWT Handshake Authentication

Many developers leave WebSocket connection lines unauthenticated. 
Anyone who knows a document's ID string could connect directly to the WebSocket server port and intercept typing streams or edit content without ever logging in!

### The Solution:
We intercept incoming connections using Socket.io's **`.use()` Middleware**. We read the client's HTTP cookies, verify the JWT, and attach the user profile directly to the connection socket before executing the connection handshake.

---

## 2. Implementing Socket Handshake Middleware

### Creating Auth Handshake Validator (`sockets/socketHandler.js`)
Here is the secure, hardened implementation of `backend/sockets/socketHandler.js` containing the JWT handshake decoder:
...
## 3. API Rate Limiting Defense

To prevent malicious bot scripts from flooding your REST API database and causing Denial of Service (DoS) attacks, introduce a local **Express Rate Limiter**.

1. Install the npm package in your `backend/` directory:
   ```bash
   npm install express-rate-limit
   ```
2. Inject it into your Express bootstrap within `backend/server.js`:
   ```javascript
...
Your collaborative editor is now extremely secure and resilient against injection attacks, session hijacks, and scraping attempts! Next, we will cover testing models and sockets.
